...
namespace ShoeStore.DAL
{
    ...
    internal class DbHelperSQL
    {
        ...
        /// <returns></returns>
        public static DataTable GetPageListByProc(string proName,int pageIndex, int pageSize, out int pageCount, out int num)
        {
            ...
            using (SqlConnection conn = new SqlConnection(strConn))
            {
                ...
                da.SelectCommand.Parameters.Add(new SqlParameter("@Num",SqlDbType.Int));
                //将后面两个参数 设置为 输出类型
                ...
                pageCount = Convert.ToInt32(da.SelectCommand.Parameters[2].Value);
                 num= Convert.ToInt32(da.SelectCommand.Parameters[3].Value);
            }
            //返回数据表
            return dt;
        } 
        ...
        /// <returns></returns>
        public static DataTable GetBuyDay(string proName,int day,int top)
        {
            DataTable dt = new DataTable();
            //创建连接通道
            using (SqlConnection conn = new SqlConnection(strConn))
            {
                ...
                da.SelectCommand.CommandType = CommandType.StoredProcedure;
                //设置 参数
                da.SelectCommand.Parameters.AddWithValue("@day", day);//选择是查找最近多久的数据
                da.SelectCommand.Parameters.AddWithValue("@top", top);// 选择是查找前几条
                //执行 并将查询到的 结果 赋给 数据表对象
                da.Fill(dt);
            }
            //返回数据表
        ...
        public static DataTable GetSearch(string proName,string word, int pageIndex, int pageSize, out int pageCount, out int num)
        {
            DataTable dt = new DataTable();
            //创建连接通道
            using (SqlConnection conn = new SqlConnection(strConn))
            {
                ...
                num = Convert.ToInt32(da.SelectCommand.Parameters[4].Value);
            }
            //返回数据表
            return dt;
        }

        public static DataTable GetProByCate(string proName, int id, int pageIndex, int pageSize, out int pageCount, out int num)
        {
            DataTable dt = new DataTable();
            //创建连接通道
            using (SqlConnection conn = new SqlConnection(strConn))
        ...
        public static DataTable Getproc(string proName,  int pageIndex, int pageSize, out int pageCount, out int num)
        {
            return GetProByCate(proName,0, pageIndex, pageSize, out pageCount, out num);
        }
    }
}
