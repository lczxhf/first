<%@ Page Title="" Language="C#" MasterPageFile="~/manage/View/common.Master" AutoEventWireup="true" CodeBehind="CarItems.aspx.cs" Inherits="ShoeStore.Web.manage.View.CarItems" %>
<asp:Content ID="Content1" ContentPlaceHolderID="placeHead" runat="server">
       <script type="text/javascript">
           var targetUrl = "/manage/Action/CarItems.ashx";
           //当前正在被操作的 行dom对象
           var $nowRow = null;
           $(function () {
               //1.为新增按钮绑定方法
           ...
           function showEditPanel(mid, btn) {
               ...
               $.get(targetUrl, { type: "get", "mid": mid, s: Math.random() }, function (jsObj) {
                   ...
                   processData(jsObj, function () {
                       ...
                       $("#pName").val(jsObj.data.CPIdMODEL.PName);
                       $("#Count").val(jsObj.data.CCount);
                       $("#Time").val(jsonDateFormat(jsObj.data.CTime));
                       $("#UserName").val(jsObj.data.CUserMODEL.ULoginName);
                       $("#MId").val(jsObj.data.CItemId);
                       //3.展开 修改面板
               ...
               $.post(targetUrl, data + "&type=m", function (jsObj) {
                   ...
                   processData(jsObj, function () {
                       ...
                       if ($nowRow != null) {
                           ...
                           $tds[2].innerHTML = jsObj.data.CPIdMODEL.PName;
                           $tds[3].innerHTML = jsObj.data.CCount;
                           $tds[4].innerHTML = jsonDateFormat(jsObj.data.CTime);
                           $tds[1].innerHTML = jsObj.data.CUserMODEL.ULoginName;
                           //清空选中行
                           $nowRow = null;
                       }
                   });
...
<asp:Content ID="Content2" ContentPlaceHolderID="placeRight" runat="server">
                <table id="tbList">
    <tr id="trNav" ><th colspan="5">当前正在：【购物车】管理</th><th><input type="button" id="btnAdd" value="新增" /></th></tr>
    <tr>
        <th>Id</th>
        <th>用户</th>
        <th>商品</th>
    ...
    <asp:Repeater runat="server" ID="rptList">
        <ItemTemplate>
            <tr>
            <td><%#Eval("CItemId")%></td>
            <td><%#Eval("CUserMODEL.ULoginName")%></td>
            <td><%#Eval("CPIdMODEL.PName")%></td>
            <td><%#Eval("CCount")%></td>
            <td><%#Eval("CTime")%></td>
            <td>
                <a href="javascript:void(0)" onclick="doDel(<%#Eval("CItemId")%>,this)">删</a> 
                <a href="javascript:void(0)" onclick="showEditPanel(<%#Eval("CItemId")%>,this)">改</a>
            </td>
        </tr>
        </ItemTemplate>
    </asp:Repeater>
